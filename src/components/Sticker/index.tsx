...
import close from "@/assets/images/icon_Sticker_Close.png";
...
class Sticker extends Component {
  ...
  constructor(props) {
    ...
    this.throttledArrowOntouchmove = this.throttle(
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let hasChange = false;
    if (
      nextProps.framePrams &&
      nextProps.framePrams.width !== this.props.framePrams.width
    ) {
  ...
  arrowOntouchstart = (e) => {
    ...
    const frameOffsetX = framePrams.left;
    const frameOffsetY = framePrams.top;
    const center = tool.calcCenterPosition(
      stylePrams.x,
      stylePrams.y,
      stylePrams.width,
      stylePrams.height
      ...
      <View>
        <View
          className={`sticker-wrap ${
            stylePrams.fixed || !stylePrams.isActive ? "event-through" : ""
          } ${stylePrams.visible && stylePrams.width > 0 ? "" : "hidden"} ${
            stylePrams.isActive ? "canChange" : ""
          }`}
          style={styleObj}
        >
          ...
          <View
            className={`border ${stylePrams.isActive ? "active" : ""}`}
          ></View>
          <View
        ...
        <View
          className={`sticker-wrap copy event-through ${
            stylePrams.visible && stylePrams.width > 0 ? "" : "hidden"
          } ${stylePrams.isActive ? "canChange" : ""}`}
          style={styleObj}
        >
